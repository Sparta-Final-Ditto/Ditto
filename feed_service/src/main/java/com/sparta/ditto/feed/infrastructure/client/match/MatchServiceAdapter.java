...
public class MatchServiceAdapter implements MatchServicePort {
    ...
    private static final String CACHE_KEY_PREFIX = "feed:reco:";
    ...
    public MatchServiceAdapter(
            MatchServiceClient matchServiceClient,
            StringRedisTemplate redisTemplate,
            ObjectMapper objectMapper,
            @Value("${app.cache.recommendations-ttl:10m}") Duration ttl) {
    ...
    public RecommendationResult getRecommendations(UUID userId, int limit) {
        ...
        if (cached != null) {
            return new RecommendationResult(cached);
        }
    ...
    private void writeCache(String cacheKey, List<UUID> userIds) {
        try {

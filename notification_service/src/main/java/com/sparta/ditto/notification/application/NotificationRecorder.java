...
public class NotificationRecorder {
    ...
    public void recordPost(PostNotificationCommand cmd) {
        ...
        // 멱등 성공 skip: 이미 처리된 이벤트면 저장·전송 없이 정상 종료 (사전 exists 체크, TRD 10장)
        if (notificationRepository.existsByTypeAndTargetIdAndReceiverId(
                type, cmd.targetId(), cmd.ownerId())) {
            log.info("중복 알림 skip (멱등, 사전 exists): type={}, targetId={}, receiverId={}",
                    type, cmd.targetId(), cmd.ownerId());
            return;
        }
    ...
    public void recordChat(ChatNotificationCommand cmd) {
        ...
        for (UUID receiverId : cmd.receiverIds()) {
            // 수신자별 멱등 성공 skip: 이미 있는 수신자만 건너뛰고 나머지는 같은 트랜잭션에서 저장
            if (notificationRepository.existsByTypeAndTargetIdAndReceiverId(
                    NotificationType.CHAT_MESSAGE, cmd.messageId(), receiverId)) {
                log.info("중복 채팅 알림 skip (멱등, 사전 exists): messageId={}, receiverId={}",
                        cmd.messageId(), receiverId);
                continue;
            }
            Notification notification = Notification.create(

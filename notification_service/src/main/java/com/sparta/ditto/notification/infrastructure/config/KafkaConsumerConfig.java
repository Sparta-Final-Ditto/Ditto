 ...
 * <ul>
 *   <li>일시적 오류는 총 3회(최초 1회 + 재시도 2회, FixedBackOff 1초) 시도 후 DLT로 발행하고
 *       offset을 커밋한다.</li>
 *   <li>DLT 토픽은 "&lt;원본토픽&gt;.&lt;컨슈머그룹&gt;.DLT"로 라우팅한다(post-events는 다른 컨슈머
 *       그룹도 구독하므로 Spring 기본 "&lt;토픽&gt;.DLT"를 쓰면 서비스 간 실패 메시지가 섞인다).
 *       파티션은 미지정한다.</li>
 *   <li>not-retryable 예외(JsonProcessingException/IllegalArgumentException)는 재시도 없이
 *       즉시 DLT로 보낸다.</li>
 *   <li>DataIntegrityViolationException(멱등 성공)은 재시도·DLT 모두 없이 로그 후 정상 종료한다.</li>
 *   <li>DLT 발행 시 원본 topic/partition/offset·예외를 Kafka 헤더로 보존하고, 발행 전 ERROR 로그에
 *       스택트레이스를 남긴다.</li>
 * </ul>
 * consumer record value가 String이므로 DLT 재발행 producer도 StringSerializer로 고정한다
 * (JsonSerializer는 이중 인코딩).
 */
@Configuration
public class KafkaConsumerConfig {
    ...
    public NewTopic postEventsDltTopic() {
        return TopicBuilder
                .name(NotificationDltDestinationResolver.dltTopicName(
                        "post-events", consumerGroupId))
                .partitions(1)
    ...
    public NewTopic chatMessageCreatedDltTopic() {
        return TopicBuilder
                .name(NotificationDltDestinationResolver.dltTopicName(
                        "chat-message-created", consumerGroupId))
                .partitions(1)
                .config(TopicConfig.RETENTION_MS_CONFIG, String.valueOf(RETENTION_14_DAYS_MS))
                .build();
    }
